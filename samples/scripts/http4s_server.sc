...
class AppComponents(config: AppConfig):
  ...
  def createServices(xa: HikariTransactor[IO]): AppServices[IO] = {
    // インフラストラクチャ層
    val logger:          Logger[IO]          = infrastructure.Logger.console[IO]
    val staffRepository: StaffRepository[IO] = new StaffRepositoryImpl(xa)
    val eventPublisher:  EventPublisher[IO]  = new AwsSnsEventPublisher(snsClient, config.snsTopicArn, logger)
    val idGenerator:     IdGenerator[IO]     = infrastructure.IdGenerator.uuid[IO]
    val timeProvider:    TimeProvider[IO]    = infrastructure.TimeProvider.system[IO]
    ...
    AppServices(createStaffService)
  }
  ...
  def run: IO[Unit] =
    ...
    components.transactorResource.use { xa =>
      ...
      for
        ...
        _ <- IO.println("Starting Staff Management API Server...")
